...
class Network:
    ...
    def __init__(self, params, activate, activate_derivative):
        ...
        self.biases = [np.random.uniform(low=-math.sqrt(6/(self.params[y-1] + self.params[y])),
                                         high=math.sqrt(6/(self.params[y-1] + self.params[y])),
                                         size=(self.params[y], 1)) for y in range(1, self.layer_num)]
        self.weights = [np.random.uniform(low=-math.sqrt(6/(self.params[y] + self.params[y-1])),
                                          high=math.sqrt(6/(self.params[y] + self.params[y-1])),
...
net = Network([784, 30, 30, 30, 30, 10], activate=sigmoid, activate_derivative=sigmoid_prime)
net.train(training_data=training_data, epochs=30, mini_batch_size=10, lr=0.3, test_data=test_data)
plt.plot(Epoch, Loss, color='red')
plt.title('3rd experiment')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.show()